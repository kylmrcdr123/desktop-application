...
public class SearchHistoryController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        if (student != null) {
            ...
            List<CommunityService> communityServiceByStudName = communityServiceFacade.getAllCsByStudent(student);

            int totalHoursCompleted = computeTotalHoursCompleted(communityServiceByStudName);
            totalField.setText(String.valueOf(totalHoursCompleted));

            ObservableList<CommunityService> data = FXCollections.observableArrayList(communityServiceByStudName);
            table.setItems(data);
        } else {
    ...
    private int computeTotalHoursCompleted(List<CommunityService> communityServiceByStudName) {
        int totalHoursCompleted = 0;
        for (CommunityService communityService : communityServiceByStudName) {
            totalHoursCompleted += communityService.getHours_completed();
        }
